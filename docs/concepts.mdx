...
<Steps>
  ...
  <Step title="Shapes the response">
    Success or a consistent error envelope — no stack traces leak to the caller.
  </Step>
</Steps>
...
**`DataSourceManager`** can sequence multiple adapters with **failover** and **circuit breaking**, so a
flaky upstream degrades gracefully instead of taking the tool down.
